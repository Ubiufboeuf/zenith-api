...
import { DB, LOCAL_DATABASE_URL } from '@/lib/constants/env'

const db_url = DB === 'local' ? LOCAL_DATABASE_URL : 'http://localhost:7103'
...
export async function api (endpoint: string | URL | Request, options?: RequestInit & { silence?: boolean }) {
  ...
  return fetch(`${db_url}${endpoint}`, processedOptions)
    .then(async (res) => {
      if (!res.ok) {
